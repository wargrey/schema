...
(define pk64:timestamp : (->* () (Integer) Natural)
  (lambda [[diff:s 0]]
    (define version : Byte #b001)
    (define now:ms : Flonum (current-inexact-milliseconds)) ; milliseconds is not enough when generating a list of UUPKs.
    ...
    (abs (bitwise-ior (arithmetic-shift version 60)
                      (arithmetic-shift ts32 28)
                      (fxlshift ms20 8)
                      clock-seq8))))

(define pk64:random : (-> Natural)
  (lambda []
