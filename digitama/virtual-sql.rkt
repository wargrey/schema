...
(define create-table.sql : (-> Boolean String (Listof+ String) (Option String) (Listof String) (Listof String)
  ...
  (lambda [silent? table pks eam cols types not-nulls uniques]
    (virtual-statement
     (λ [[dbms : DBSystem]]
       (define without-rowid? : Boolean #false)
       (define single-pk : (Option String) (and (null? (cdr pks)) (car pks)))
       (define (++++ [name : String] [type : String] !null uniq) : String
         (define name+type : String (string-append name " " type))
         (when (and (not without-rowid?) (member name pks)) (set! without-rowid? #true))
       ...
       (case (dbsystem-name dbms)
         [(sqlite3)
          (format "CREATE ~a ~a (~a~a~a)~a;"
                  ...
                  (if single-pk "" (format ", PRIMARY KEY (~a)" (string-join pks ", ")))
         ...
         [(sqlite3)
          (define-values (snmuloc seulva$)
            (for/fold ([snmuloc : (Listof String) null] [seulva$ : (Listof String) null])
                      ([col (in-list (append (cdr cols) (if eam (list eam) null)))]
                       [idx (in-naturals 2)])
...
(define update.sql : (-> String (Listof+ String) (Option String) (Listof String) Virtual-Statement)
  (lambda [table pks eam cols]
    (virtual-statement
     (λ [[dbms : DBSystem]]
       (case (dbsystem-name dbms)
         [(sqlite3)
          (format "UPDATE ~a SET ~a WHERE ~a;" table
                  (string-join=$i (append cols (if eam (list eam) null)) ", " (length pks))
                  (string-join=$i pks " AND " 0))]
...
(define simple-select.sql : (-> Symbol String (Listof String) (Option String) (Listof String) Virtual-Statement)
  (lambda [which table pks eam cols]
    (define ~pk : String "SELECT ~a FROM ~a WHERE ~a;")
    (virtual-statement
     (case which
       [(nowhere) (format "SELECT ~a FROM ~a;" (or eam (string-join pks ", ")) table)]
       [(byrowid) (λ [[dbms : DBSystem]] (format ~pk (or eam (string-join pks ", ")) table (string-join=$i pks " AND " 0)))]
       [(ckrowid) (λ [[dbms : DBSystem]] (format ~pk (car pks) table (string-join=$i pks " AND " 0)))]
       [else #|row|# (λ [[dbms : DBSystem]] (format ~pk (string-join cols ", ") table (string-join=$i pks " AND " 0)))]))))

(define delete.sql : (-> String (Listof String) Virtual-Statement)
  (lambda [table pks]
    (virtual-statement (λ [[dbms : DBSystem]] (format "DELETE FROM ~a WHERE ~a;" table (string-join=$i pks " AND " 0))))))
